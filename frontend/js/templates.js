const navbarTemplate = Handlebars.compile(
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-light">
        <a class="navbar-brand mb-0 h1">COTA</a>
        ...
        <ul class="navbar-nav mr-auto">
            ...
            <li class="nav-item font-weight-bold">
            <a class="nav-link" href="#series/popular">Popular Series<span class="sr-only">(current)</span></a>
            </li>
        </ul>
        <form class="form-inline my-2 my-lg-0">
            <input id="searchSeriesInput" class="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search">
            <button id="searchSeriesBtn" class="btn btn-outline-success my-2 my-sm-0" type="button">Search</button>
...
const seriesTemplate = Handlebars.compile(
    `
    <div class="content-box">
        ...
        <p>{{{desc}}}</p>
        <div id="seriesInfoFooter" class="clearfix">
            ...
            <p class="lead float-left text-muted">{{min}}-{{max}} mins</p>
        </div>
    </div>
    `
)

const MultipleSeriesTemplate = Handlebars.compile(
    `
    <div class="content-box">
        <table class="table">
            <thead>
...
const popularSeriesTemplate = Handlebars.compile(
    `
    <div class="content-box">
        <table class="table">
            <thead>
    ...
    <div class="content-box">
        <div class="mt-3">
            ...
            <table class="table mt-3">
                ...
                {{#each series}}
                <tr>
                    <td id="SeriesName" scope="row">
                        <img src="{{image}}" style="border-radius: 50%; width: 40px; height: 40px; margin-right:10px;"/>
                        <a id="seriesNameHref" href="#series/{{name}}">{{name}}</a>
                        
                    </td>
                    
                    <td>{{rating}}</td>
                <td><i id="deleteSeries" class="fas fa-trash" style="color:red; cursor:pointer;"></i></td>
                </tr>
                {{/each}}
                </tbody>
            </table>
...
const filterBetweenModalTemplate = Handlebars.compile(
    `
    <div id="filterBetweenModal" class="modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form>
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="minInput" placeholder="Lower limit">
                    </div>
                    <div class="form-group">
                        <label for="maxInput">Maximum</label>
                        <input type="number" class="form-control" id="maxInput" placeholder="Upper limit">
                    </div>
                </form>
            </div>
...
const addSeriesToGroupBtnTemplate = Handlebars.compile(
    `
    <p id="addSerieToGroupBtn" data-toggle="modal" data-target="#listGroupsModal" class="lead float-right text-muted" style="cursor: pointer"><u>Add game to group</u></p>
    `
)

const filterBetweenBtnTemplate = Handlebars.compile(
    `
    <button id="filterBetweenBtn" type="submit" data-toggle="modal" data-target="#filterBetweenPlaytimeModal" class="lead float-right btn btn-primary btn-sm" style="cursor: pointer"><i class="fas fa-sliders-h"></i></button>
    `
)